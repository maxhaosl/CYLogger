...
set ARCH_LIST=%1
if "%ARCH_LIST%"=="" set ARCH_LIST=x86_64
set LIB_LIST=%2
...
REM Use OUTPUT_ARCH for output directory paths
set ARCH_DIR=%PROJECT_ROOT%\Bin\Windows\!OUTPUT_ARCH!\!RUNTIME_FOR_CONFIG!\%CONFIG%
set NEED_STATIC=0
if not exist "%ARCH_DIR%\CYCoroutine.lib" set NEED_STATIC=1

REM CYCoroutine only supports static library, not shared library
REM Always check and build static library only, regardless of KIND parameter
if %NEED_STATIC%==0 (
    echo CYCoroutine static library already present for !OUTPUT_ARCH! / %CONFIG% / !RUNTIME_FOR_CONFIG!
    endlocal & exit /b 0
)

REM Always build CYCoroutine as static library (BUILD_SHARED_LIBS=OFF)
...
echo Building CYCoroutine_static for !OUTPUT_ARCH! / %CONFIG% / !RUNTIME_FOR_CONFIG!...
