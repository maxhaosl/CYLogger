...
CCYLoggerTestDlg::CCYLoggerTestDlg(CWnd* pParent /*=nullptr*/)
    : CDialogEx(IDD_CYLOGGERTEST_DIALOG, pParent)
    , m_bRunning(false)
{
    m_hIcon = AfxGetApp()->LoadIcon(IDR_MAINFRAME);
}

void CCYLoggerTestDlg::DoDataExchange(CDataExchange* pDX)
{
    CDialogEx::DoDataExchange(pDX);
}

BEGIN_MESSAGE_MAP(CCYLoggerTestDlg, CDialogEx)
    ON_WM_SYSCOMMAND()
    ON_WM_PAINT()
    ON_WM_QUERYDRAGICON()
    ON_BN_CLICKED(IDOK, &CCYLoggerTestDlg::OnBnClickedOk)
    ON_BN_CLICKED(IDC_BTN_START, &CCYLoggerTestDlg::OnBnClickedBtnStart)
    ON_BN_CLICKED(IDC_BTN_STOP, &CCYLoggerTestDlg::OnBnClickedBtnStop)
    ON_WM_CLOSE()
END_MESSAGE_MAP()


// CCYLoggerTestDlg 消息处理程序

BOOL CCYLoggerTestDlg::OnInitDialog()
{
...
void CCYLoggerTestDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
//显示。
HCURSOR CCYLoggerTestDlg::OnQueryDragIcon()
{
    return static_cast<HCURSOR>(m_hIcon);
}

void CCYLoggerTestDlg::OnBnClickedOk()
{
    // TODO: 在此添加控件通知处理程序代码
    CDialogEx::OnOK();
}

void CCYLoggerTestDlg::Entry()
{
//     CY_TRACE();
//     CY_SCOPE();
...
void CCYLoggerTestDlg::OnBnClickedBtnStart()
{
    if (m_bRunning)
    {
        return;
    ...
    m_bRunning = true;
    m_thread = std::jthread(&CCYLoggerTestDlg::Entry, this);
}


void CCYLoggerTestDlg::OnBnClickedBtnStop()
{
    m_bRunning = false;
